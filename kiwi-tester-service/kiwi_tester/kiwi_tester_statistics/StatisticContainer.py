...
class StatisticContainer:
    ...
    def add_testing_result(self, user, item, vote, prediction, recommender, activation, duration):
        self.testing_results.append(
            (user, item, vote, prediction, recommender, activation, duration))
    ...
    def counts_of_single_recommenders(self):
        counts = {}
        for _, _, _, _, rec, _, _ in self.testing_results:
            if counts.get(rec):
                counts[rec] +=1
                continue
            counts[rec] = 1
        self.recommender_counts = counts
